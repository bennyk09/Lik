...
async function renderSwapsDashboard(myUid) {
    try {
        ...
        // 2. POPULATE SENT REQUESTS (PENDING STATUS)
        ...
        mutualContainer.innerHTML = mutualIds.length === 0 ? `<p style="color: var(--text-muted); font-size: 0.85rem; padding: 8px 4px;">No mutual swapped networks formed yet.</p>` : "";
...
// 🪐 NEW: Search Node Element Registries for Swaps Page
...
// 🪐 NEW MODULE: Real-Time Handle Prefix Search Engine on the Swaps Dashboard Page
if (searchInput) {
    searchInput.oninput = async (e) => {
        ...
        try {
            ...
            snapshot.forEach(docData => {
                ...
                // Clicking any option routes you straight to their native timeline profile screen!
                row.onclick = () => {
    ...
    // Auto-close floating dropdown search tray when user clicks outside the component boundaries
